...
static inline lv_obj_t *endpoint_status_create(lv_obj_t *parent) {
  ...
  lv_obj_set_size(container, 39, 12);

#if IS_ENABLED(CONFIG_ZMK_WIDGET_BLE_STATUS_16X12)
  lv_ble_status_create(container, container_default, LV_ALIGN_CENTER);
#endif
  ...
  lv_usb_status_create(container, container_default, LV_ALIGN_CENTER);
#endif
...
static inline lv_obj_t *status_bar_create(lv_obj_t *parent) {
  ...
  lv_obj_t *indicators = lv_hid_indicators_create(container, container_default, LV_ALIGN_CENTER);
...
lv_obj_t *zmk_display_status_screen() {
  ...
  zmk_status_presenter_dispatch(container, 3);

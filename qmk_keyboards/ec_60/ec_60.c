...
#define id_custom_channel_apc id_custom_channel_user_range

deferred_token ec_debug_token;  // defer_exec token
...
static uint32_t defer_ecsm_send_matrix_cb(uint32_t trigger_time, void *cb_arg) {
  ecsm_send_matrix();
  ec_debug_token = 0;
  return 0;
}
  ...
  switch (keycode) {
    case EC_DBG:
      if (!record->event.pressed) {
        ec_debug_token = defer_exec(1000, &defer_ecsm_send_matrix_cb, NULL);
        return false;
      }
  }
  return true;
...
void apc_config_save(void) { eeconfig_update_user_datablock(&apc); }
...
bool via_custom_value_command_user(uint8_t *data, uint8_t length) {
  ...
  if (*channel_id == id_custom_channel_apc) {
    switch (*command_id) {
